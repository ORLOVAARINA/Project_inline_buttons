...
async def process_start_command(message: Message):
    ...
    await message.answer(
        text='Это инлайн-клавиатура! Выбери кнопку:',
...
async def process_proverb(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(callback_query.from_user.id, f'Вот пословица: "{random_proverb}"')
...
async def process_message(callback_query: types.CallbackQuery):
    await bot.send_message(callback_query.from_user.id, 'Твое сообщение')
